...
class PDF(FPDF):
    ...
    def header(self):
        ...
        with self.table(
            ...
            row.cell(
                "PETUNJUK PELAKSANAAN PEMUATAN KARGO UMUM (IMPLEMENTATION INSTRUCTION OF CARGO LOADING)", rowspan=3)
            row.cell("REV.")
            row.cell("00")
            ...
            row = table.row()
            row.cell(
                f"{self.uuid}/IFF/{datetime.datetime.now().strftime('%Y')}", colspan=2)
    ...
    def first_page(self):
        ...
        with self.table(borders_layout="NONE",
            ...
            row.cell(
                f": {self.vehicle_box.size[0]} x {self.vehicle_box.size[1]} x {self.vehicle_box.size[2]} cm")
        ...
        self.numbering(3, "DejaVu", u'\u2022',
                       "Pastikan kontainer dalam kondisi baik, bersih, dan kering.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Periksa adanya kerusakan yang bisa mempengaruhi keamanan cargo.")
        self.numbering(2, "Times", "2.", "Perlengkapan Pemuatan:")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Pastikan semua alat pemuatan seperti forklift, jack, tali, dan selimut pelindung tersedia dan dalam kondisi baik.")

        self.numbering(1, "Times", "II.", "Penyusunan Cargo")
        self.numbering(2, "Times", "1.", "Perencanaan Penyusunan:")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Susun rencana penyusunan cargo berdasarkan berat, ukuran, dan sensitivitas barang.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Pastikan barang berat tidak diletakkan di atas barang ringan. Form rencana susunan kargo terlampir.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Gunakan prinsip 'Last In, First Out' (LIFO) untuk barang yang dibutuhkan pertama kali di tujuan (Jika diperlukan).")
        self.numbering(2, "Times", "2.", "Penyusunan Barang:")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Mulai dengan barang terberat diletakkan di lantai kontainer.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Barang yang lebih ringan bisa disusun di atasnya, pastikan stabilitas.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Gunakan bahan pengisi (dunnage) untuk menghindari pergerakan barang selama transit.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Posisi penyusunan kargo harus sesuai dengan perencanaan")
        self.numbering(2, "Times", "3.", "Pengikatan dan Pengamanan:")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Gunakan tali dan ikatan untuk mengamankan barang.")
        self.numbering(3, "DejaVu", u'\u2022',
                       "Pastikan tidak ada ruang kosong yang bisa menyebabkan barang bergeser.")

        self.numbering(1, "Times", "III.", "Dokumentasi")
        self.numbering(2, "DejaVu", u'\u2022',
                       "Catat setiap langkah pemuatan, termasuk posisi dan pengamanan barang.")
        self.numbering(2, "DejaVu", u'\u2022',
                       "Ambil foto sebagai bukti kondisi barang dan penyusunannya sebelum menutup kontainer.")

        self.numbering(1, "Times", "IV.", "Penutupan dan Segel")
        self.numbering(2, "DejaVu", u'\u2022',
                       "Pastikan semua pintu kontainer tertutup dengan baik.")
        self.numbering(2, "DejaVu", u'\u2022',
                       "Pasang segel keamanan dan catat nomor segel untuk dokumen pengiriman.")

        self.numbering(1, "Times", "V.", "Peninjauan Akhir")
        self.numbering(2, "DejaVu", u'\u2022',
                       "Lakukan peninjauan akhir untuk memastikan semua SOP telah diikuti.")
        self.numbering(2, "DejaVu", u'\u2022',
                       "Tandatangani dan berikan formulir ini kepada pihak yang bertanggung jawab atas pengiriman.")
    ...
    def second_page(self):
        ...
        headings_style = FontFace(
            emphasis="BOLD", color=0, fill_color=(115, 230, 255))
        with self.table(
            borders_layout="ALL",
            # cell_fill_color=(224, 235, 255),
            # cell_fill_mode=TableCellFillMode.ROWS,
            ...
            for insertion_order, item in enumerate(self.vehicle_box.packed_items):
                ...
                row.cell(
                    f'{item.size[0]} x {item.size[1]} x {item.size[2]} cm')
                row.cell(str(item.weight))
                row.cell(
                    f'{item.position[0]}, {item.position[1]}, {item.position[2]} cm')
            ...
            if isinstance(item, Box):
                ...
                with self.table(
                    ...
                    for insertion_order, item2 in enumerate(item.packed_items):
                        ...
                        row.cell(
                            f'{item2.size[0]} x {item2.size[1]} x {item2.size[2]} cm')
                        row.cell(str(item2.weight))
                        row.cell(
                            f'{item2.position[0]}, {item2.position[1]}, {item2.position[2]} cm')

                self.ln(15)

        self.write(text="Diagram/Sketsa Penyusunan:\n")
        self.write(
            text="(Sertakan gambar atau sketsa untuk memperjelas penyusunan dan posisi cargo)\n")
        ...
        self.write(
            text=f"Tanggal: {datetime.datetime.now().strftime('%d/%m/%Y')}\n\n")
